...
export default function ChatBox() {
  ...
  useEffect(() => {
    const updateOnlineUsers = (message: any) => {
      if (message.type === messageType.ONLINE_COUNT) {
        setOnlineUsers(message.onlineUsers);
      }
    };

    if (webSocketState.messages) {
      webSocketState.messages.forEach(updateOnlineUsers);
    }
